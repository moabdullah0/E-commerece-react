...
const Productdetailes = () => {
  ...
  const [Errors, setError] = useState(''); // Initial quantity is set to 1
  const { id } = useParams();

  useEffect(() => {
    axios.get(`http://127.0.0.1:8000/api/product-detailes/${id}`).then((response) => {
  ...
  <Navbar/>
      ...
      <h1 className='items-center bg-red-300 w-56 rounded-full text-center font-bold text-xl text-gray-200 mb-10'>{Errors}</h1>
